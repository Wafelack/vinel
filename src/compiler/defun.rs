...
impl Compiler {
    pub fn defun(&mut self, args: Vec<Expr>) -> Result<String, String> {
        ...
        for arg in args.clone() {
            if let ExprT::Symbol(sym) = &arg.exprt {
                ...
                if let ExprT::Call(n, args) = arg.exprt {
                    ...
                    return Err(format!("{}:{} | Expected name and arguments definiton (Function Call), found {}.", arg.line, arg.column, arg.get_type()));
                }
            }  else {
        ...
        if name.is_none() {
            ...
            return Err(format!("{}:{} | Function name has not been defined.", last.line, last.column));
        }


        Ok(
            format!("function{} {}({}){}\n{}endfunction", if no_overwrite { "" } else { "!" }, if script { format!("s:{}", name.unwrap()) } else { name.unwrap() }, func_args.join(", "), if abort { " abort" } else { "" }, body)
          )
    }
}
