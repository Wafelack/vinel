...
impl Compiler {
    pub fn get(&mut self, args: Vec<Expr>) -> Result<String, String> {
        ...
        if !(0..=2).contains(&args.len()) {
            return Err(format!("Function `get` takes 0, 1 or 2 arguments, but {} arguments were supplied.", args.len()));
        } 
        ...
        let (option, newline, termcap, all) = if args.len() == 0 {
            ...
            if let ExprT::Symbol(sym) = &args[1].exprt {
                match sym.as_str() {
                    "all" => if all {
                        return Err(format!("{}:{} | all: Symbol is already present.", &args[1].line, args[1].column));
                    } else {
                        all = true
                    },                                "termcap" => if termcap{
                        return Err(format!("{}:{} | termcap: Symbol is already present.", &args[1].line, args[1].column));
                    } else {
                        termcap = true
                    },                    
                        "newline" => if newline {
                            return Err(format!("{}:{} | newline: Symbol is already present.", &args[1].line, args[1].column));
                        } else {
                            newline = true
                        },
        ...
        if termcap && all {
            return Err(format!("Symbols `termcap` and `all` cannot be present simultaneously."));
        }

        Ok(format!("set{}{}", if newline { "!" } else { "" }, if all { " all".to_string() } else if termcap { " termcap".to_string() } else if option.is_some()  { format!(" {}?", option.unwrap()) } else { "".to_string() }))
    }
}
